...
def ask_multiline(prompt: str, default: str = "无") -> str:
    print(prompt)
    print("(支持多行输入；直接回车结束。)")
    lines = []
    while True:
        ...
        for raw in f:
            ...
            if section == "weeks":
                ...
                if current_week is not None and line.startswith("      "):
                    k, v = line.strip().split(":", 1)
                    parent = None
                    for wk_key in ("p1_note", "p1_exp", "p1_qa", "p2", "p3", "resume"):
                        if isinstance(data["weeks"][current_week].get(wk_key), dict):
                            if k.strip() == "task" and "task" not in data["weeks"][current_week][wk_key]:
                                parent = wk_key
                                break
                            if k.strip() == "est_h" and "est_h" not in data["weeks"][current_week][wk_key]:
                                parent = wk_key
                                break
                    if parent:
                        val = v.strip().strip('"')
                        if k.strip() == "est_h":
                            data["weeks"][current_week][parent]["est_h"] = float(val)
                        else:
                            data["weeks"][current_week][parent]["task"] = val
                    continue
                if current_week is not None and line.startswith("    "):
                    stripped = line.strip()
                    if stripped.endswith(":"):
                        k = stripped[:-1]
                        data["weeks"][current_week][k] = {}
                        continue
...
def ensure_default_deliverables(conn: sqlite3.Connection, ws: str, week_item: Dict) -> None:
    ...
    if existing:
        return
    state = conn.execute("SELECT deadline FROM weekly_state WHERE week_start=?", (ws,)).fetchone()
    h0_seed = (state["deadline"] if state and state["deadline"] else "") or week_item.get("d_h0", "无")
    conn.execute(
        """INSERT INTO forced_deliverables(week_start, p1_note, p1_exp, p3_exp, resume, h0_delivery)
        VALUES(?,?,?,?,?,?)""",
        (
            ws,
            week_item.get("d_p1_note", week_item.get("p1_note", {}).get("task", "")),
            week_item.get("d_p1_exp", week_item.get("p1_exp", {}).get("task", "")),
            week_item.get("d_p3_exp", week_item.get("p3", {}).get("task", "")),
            week_item.get("d_resume", week_item.get("resume", {}).get("task", "")),
            h0_seed,
        ),
    )
...
def cmd_init(_: argparse.Namespace) -> None:
    ...
    with get_conn() as conn:
        ...
        deadline = ask_multiline("工作deadline/阻塞(无则填'无'):", "无")
        p1 = input("P1当前进度(默认0): ").strip() or "0"
        ...
        if row:
            conn.execute("UPDATE forced_deliverables SET h0_delivery=? WHERE week_start=?", (deadline, ws))
    print("✅ init完成")
...
def cmd_add(_: argparse.Namespace) -> None:
    ...
    with get_conn() as conn:
        ...
        candidates = [
            ("P1", f"推导笔记: {wk['p1_note']['task']}", wk["p1_note"]["est_h"]),
            ("P1", f"实验: {wk['p1_exp']['task']}", wk["p1_exp"]["est_h"]),
            ("P1", f"Q&A: {wk['p1_qa']['task']}", wk["p1_qa"]["est_h"]),
            ("P2", wk["p2"]["task"], wk["p2"]["est_h"]),
            ("P3", wk["p3"]["task"], wk["p3"]["est_h"]),
            ("P4", wk["resume"]["task"], wk["resume"]["est_h"]),
        ]
        for cat, text, est_h in candidates:
            ...
            conn.execute(
                """INSERT INTO tasks(week_start, category, task, est_hours, status, inherited_from, created_at, updated_at)
                   VALUES(?,?,?,?,'todo',NULL,?,?)""",
                (ws, cat, text, est_h, now_ts(), now_ts()),
            )
            print(f"  已加入，预计时长来自plan.yaml: {est_h}h")
        print("补充自由任务（空行结束）")
        while True:
...
def cmd_refresh_h0(_: argparse.Namespace) -> None:
    ...
    with get_conn() as conn:
        ...
        ddl = ask_multiline(f"刷新H0工作交付/阻塞[{current}]", current)
...
def cmd_list(_: argparse.Namespace) -> None:
    ...
    with get_conn() as conn:
        ...
        fd = conn.execute("SELECT * FROM forced_deliverables WHERE week_start=?", (ws,)).fetchone()
        if fd and (not fd["h0_delivery"] or fd["h0_delivery"].strip() == ""):
            conn.execute("UPDATE forced_deliverables SET h0_delivery=? WHERE week_start=?", (state["deadline"], ws))
            fd = conn.execute("SELECT * FROM forced_deliverables WHERE week_start=?", (ws,)).fetchone()
    print_header()
